...
class Reservation {
  ...
  // (C) SAVE RESERVATION
  function save ($nombre, $telefono, $email, $sucursal, $Imagen, $eventTime) {

$corr_start=correct_times($eventTime);


    // (C2) DATABASE ENTRY
    try {
      $this->stmt = $this->pdo->prepare(
        "INSERT INTO `membresias` (`Nombre`, `Telefono`, `Email`, `Fecha_Nacimiento`,  `Sucursal`, `Imagen`) VALUES (?,?,?,FROM_UNIXTIME(?),?,?)");
      $this->stmt->execute([$nombre, $telefono, $email,  $corr_start,$sucursal,$Imagen]);
     return true;
    } catch (Exception $ex) {
